...
public class HPutCardFormaPagoConCopagoUpdateRequest extends HRequest<Pago> {
    ...
    public HPutCardFormaPagoConCopagoUpdateRequest(Pago pago, Response.Listener<Pago> listener, Response.ErrorListener errorListener) {
    ...
    public byte[] getBody() {
        ...
        try {
            ...
            if (pago.formaPago != null && pago.formaPago.id != null && !pago.formaPago.id.isEmpty()) {
                ...
                if (pago.formaPago.id.equals(ConstantsUtil.EF_FORMA_PAGO)){
                ...
                if (pago.type.equals("C")) {
                    json.put("id", (pago.idCopago != -1L ? pago.idCopago : JSONObject.NULL));
                    json.put("ficha_id", (pago.cardIdCopago != -1L ? pago.cardIdCopago : JSONObject.NULL));

                    json.put("forma_pago_id", (pago.formaCopago != null && pago.formaCopago.id != null && !pago.formaCopago.id.isEmpty()) ? pago.formaCopago.id.trim() : JSONObject.NULL);
                    json.put("salud_o_copago", (pago.type != null && !pago.type.isEmpty()) ? pago.type.trim() : JSONObject.NULL);

                    if (pago.formaCopago.id.equalsIgnoreCase(ConstantsUtil.TARJETA_CREDITO_FORMA_PAGO)) {

                        json.put("tarjeta_id", (pago.cardTypeCopago != null && pago.cardTypeCopago.id != null) ? Long.valueOf(pago.cardTypeCopago.id.trim()) : JSONObject.NULL);
                        json.put("numero_tarjeta", (pago.cardPagoNumberCopago != null && !pago.cardPagoNumberCopago.isEmpty()) ? Long.valueOf(pago.cardPagoNumberCopago.trim()) : JSONObject.NULL);
                        json.put("banco_emisor_id", (pago.bankEmiterCopago != null && pago.bankEmiterCopago.id != null) ? Long.valueOf(pago.bankEmiterCopago.id.trim()) : JSONObject.NULL);
                        json.put("fecha_vencimiento", pago.validityDateCopago != null ? ParserUtils.parseDate(pago.validityDateCopago, DATE_FORMAT) : JSONObject.NULL);
                        json.put("titular_tarjeta_afiliacion", pago.titularCardAsAffiliation);

                    } else if (pago.formaCopago.id.equalsIgnoreCase(ConstantsUtil.CBU_FORMA_PAGO)){
                        json.put("cbu", (pago.cbuCopago != null && !pago.cbuCopago.isEmpty()) ? pago.cbuCopago.trim() : JSONObject.NULL);
                        json.put("banco_id", (pago.bankCBUCopago != null && pago.bankCBUCopago.id != null) ? Long.valueOf(pago.bankCBUCopago.id.trim()) : JSONObject.NULL);
                        json.put("cuenta_tipo_id", (pago.accountTypeCopago != null && pago.accountTypeCopago.id != null) ? Long.valueOf(pago.accountTypeCopago.id.trim()) : JSONObject.NULL);
                    ...
                    } else if (pago.formaPago.id.equalsIgnoreCase(ConstantsUtil.CBU_FORMA_PAGO)){
                        json.put("cbu", (pago.cbuCopago != null && !pago.cbuCopago.isEmpty()) ? pago.cbuCopago.trim() : JSONObject.NULL);
                        json.put("banco_id", (pago.bankCBUCopago != null && pago.bankCBUCopago.id != null) ? Long.valueOf(pago.bankCBUCopago.id.trim()) : JSONObject.NULL);
                        json.put("cuenta_tipo_id", (pago.accountTypeCopago != null && pago.accountTypeCopago.id != null) ? Long.valueOf(pago.accountTypeCopago.id.trim()) : JSONObject.NULL);
        ...
        if (obj instanceof JSONObject) {
            ...
            if (dic != null) {
                try {
                    ...
                    //pago.idCopago = dic.optLong("id", -1L);
                    //pago.cardId = dic.optLong("ficha_id", -1L);
                    //pago.cardIdCopago = dic.optLong("ficha_id", -1L);
