...
public class HPostAuthorizationCobranzaGetRequest extends HRequest<List<AuthorizationCobranza>> {

    private static final String TAG = "HPOST_GET_AUTH_COBRZ";
    private static final String PATH = RestConstants.HOST + RestConstants.GET_AUTHORIZATION_COBRANZA;

    private long mPaId;


    public HPostAuthorizationCobranzaGetRequest(long paId, Response.Listener<List<AuthorizationCobranza>> listener, Response.ErrorListener errorListener) {
    ...
    public byte[] getBody() {
        ...
        try {
            ...
            Log.e(TAG, "JSON GET_AUTH_PROM: " + json.toString());
    ...
    @Override
    protected List<AuthorizationCobranza> parseObject(Object obj) {

        List<AuthorizationCobranza> authArray = new ArrayList<AuthorizationCobranza>();

        if (obj instanceof JSONObject) {
            JSONObject dic = (JSONObject) ParserUtils.parseResponse((JSONObject) obj);
            if (dic != null) {
                Log.e(TAG, "GET_AUTH_PROMO RESP: " + dic.toString());
                try {
                    JSONArray jAuthArray = dic.getJSONArray("autorizaciones");
                    if (jAuthArray != null) {
                        for (int k = 0; k < jAuthArray.length(); k++) {

                            AuthorizationCobranza auth = new AuthorizationCobranza();
